import { model, stateAt, valueAt, limitsAt, interval } from './js/physics.js';
import { Timeline } from './js/animation.js';
import { Graph } from './js/graph.js';
import { setupActivities } from './js/activities.js';
...
$('liveState').innerHTML = fields.map(([key,label]) => `<div${key === 'motionState' ? ' class="wide"' : ''}><dt>${label}</dt><dd id="live-${key}"></dd></div>`).join('');
...
function render() {
  ...
  const roadWidth = document.querySelector('.road').clientWidth, carWidth = $('carSVG').getBoundingClientRect().width;
  ...
  document.querySelector('.road').setAttribute('aria-label', `Car position ${fmt(state.position, 'metres')}; ${state.direction.toLowerCase()}`);
  document.querySelectorAll('.mark').forEach(mark => { mark.style.left = `${roadX(Number(mark.dataset.position))}px`; });
  const order = isPosition ? 1 : 2, derivative = valueAt(model, t, order);
  $('primaryReadout').textContent = derivative === null ? cornerText(t, order) : `Slope = ${isPosition ? 'v = ds/dt' : 'a = dv/dt'} = ${fmt(derivative, isPosition ? 'm s⁻¹' : 'm s⁻²')}`;
  $('derivedReadout').textContent = derivative === null ? 'No filled marker at this instant. Open circles show the limits on either side.' : `At t = ${fmt(t, 's')}, graph height = ${fmt(derivative, isPosition ? 'm s⁻¹' : 'm s⁻²')}. This equals the slope of the primary graph.`;
...
for (const id of isPosition ? ['averageMode','t1','t2'] : ['areaStart','velocityArea','accelerationArea','areaMode']) {
...
document.querySelector('.concept-link').addEventListener('click',() => { $('concepts').open = true; });
...
if ('ResizeObserver' in window) {
  ...
  graphs.forEach(graph => observer.observe(graph.canvas)); observer.observe(document.querySelector('.road'));
}
setupActivities(model.id);
resize();
