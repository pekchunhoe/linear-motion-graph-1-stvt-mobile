...
export class Graph {
  ...
  draw(t, options = {}) {
    ...
    const stepY = niceStep(this.range[1] - this.range[0], (this.bottom - this.top) / 40);
    ...
    for (let time = 0; time <= this.model.end; time += stepT) {
      ...
      c.fillText(String(time), this.x(time), this.bottom + 15);
    }
    ...
    if (options.average) {
      const [a, b] = options.average, va = valueAt(this.model, a), vb = valueAt(this.model, b);
