...
export function setupActivities(id) {
  $('conceptContent').innerHTML = `<dl class="concept-list">${concepts.map(([term, text]) => `<dt>${term}</dt><dd>${text}</dd>`).join('')}</dl>`;
  ...
  showQuestion();
  if (id === 1) setupExample(); else setupABCDE();
}
